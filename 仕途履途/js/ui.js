  ...
  function showEvent(ev) {
    ...
    if (!ev.o || !ev.o.length) {
      ...
      body += ev.o.map(function (o, i) {
        return '<button class="btn-opt" data-opt="' + i + '"><span class="idx">' + (i + 1) + '.</span>' + esc(o.x) + '</button>';
      }).join('');
    }
    body += '</div></div>';
  ...
  function renderSettleStep() {
    ...
    if (st.needChoice === 'post') {
      ...
      ctrl = '<div class="divider"></div>' + st.choices.map(function (c) {
        return '<button class="btn-opt" data-sp="' + c.i + '"><span class="idx">' + (c.i + 1) + '.</span>' + esc(c.x) + '</button>';
      }).join('');
